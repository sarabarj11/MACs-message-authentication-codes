...
def public_params_for_mac_v1(m,q):
    s = randint(16,21)
    alpha = math.sqrt((s)*math.pi/(2**19))
    q = prevprime(s**3)
    m = 256
...
for message_to_authenticate in messages_to_authenticate:    
    ...
    m,q,alpha,a,M,A = public_params_for_mac_v1(m,q)
    ...
    print("The shared secret key length for our MACv1 is:", len(''.join(format(ord(char), '08b') for char in json.dumps([S1.tolist(),s1.tolist()]))))
    ...
    m,q,alpha,a,M,A = public_params_for_mac_v2(m,q)
    ...
    print("The shared secret key length for our MACv2 is:", len(''.join(format(ord(char), '08b') for char in json.dumps([S2.tolist(),s2.tolist()]))))
    bob_secret_key_v2 = KeysGen(bob_shared_secret_v2,m,q,alpha,A)
    alice_secret_key_v2 = KeysGen(alice_shared_secret_v2,m,q,alpha,A)
    # Calculate HMAC, KMAC, and our MAC
    start_time = datetime.now()
    result_hmac = hmac_sha512(secret_key, message_to_authenticate)
    end_time = datetime.now()
    duration = end_time - start_time
    print("Generate HMAC in picoseconds:", duration.total_seconds() * (10**12))
    ...
    end_time = datetime.now()
    duration = end_time - start_time
    print("Generate KMAC in picoseconds:", duration.total_seconds() * (10**12))
    ...
    end_time = datetime.now()
    duration = end_time - start_time
    print("Generate CMAC in picoseconds:", duration.total_seconds() * (10**12))
    ...
    end_time = datetime.now()
    duration = end_time - start_time
    print("Our MACv1 generation by Alice in picoseconds:", duration.total_seconds() * (10**12))
    print("Our MACv1 correctness proof:", verify_mac_v1(bob_secret_key_v1, message_to_authenticate, alice_mac_v1))
    start_time = datetime.now()
    alice_mac_v2 = generate_mac_v2(alice_secret_key_v2, message_to_authenticate)
    end_time = datetime.now()
    duration = end_time - start_time
    print("Our MACv2 generation by Alice in picoseconds:", duration.total_seconds() * (10**12))
    print("Our MACv2 correctness proof:", verify_mac_v2(bob_secret_key_v2, message_to_authenticate, alice_mac_v2))
    # Print the result
    ...
    print(f"HMAC-SHA2-512: {result_hmac}, its length is: {len(4*result_hmac)}")
    print(f"KMAC-SHA3-512: {result_kmac}, its length is: {len(4*result_kmac)}")
    print(f"CMAC-AES256-SHA2-512: {result_cmac}, its length is: {len(4*result_cmac)}")
    print(f"Our MACv1: {alice_mac_v1}, its length is: {len(4*alice_mac_v1)}")
    print(f"Our MACv2: {alice_mac_v2}, its length is: {len(4*alice_mac_v2)}")
